...
public interface UnitTypeRepo extends JpaRepository<UnitType,Long> {
    ...
    @Query(value = "SELECT Ut FROM UnitType Ut WHERE Ut.isArchived = false AND Ut.user.id = :userId")
    List<UnitType> findAllNoneArchived(Long userId);

    @Query(value = "SELECT Ut FROM UnitType Ut WHERE Ut.isArchived = false AND Ut.id = :unitTypeId AND Ut.user.id = :userId")
    Optional<UnitType> findById(Long unitTypeId, Long userId);

    @Modifying
    @Query(value = "DELETE FROM UnitType Ut WHERE Ut.id = :unitTypeId AND Ut.user.id = :userId")
